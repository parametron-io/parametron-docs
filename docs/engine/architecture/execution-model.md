...
Engine executes an ordered execution plan through product jobs. The
...
```text
execution plan -> product jobs -> scheduler workers
                                                    |
                           CAD-runtime job: validated handoff package
                           other job:       direct execution
                                                    |
                                                    v
...
Engine groups steps by product key, preserves their order within each
...
Job construction rejects empty jobs and inconsistent product identity. A job ID is the
...
For CAD-runtime work, Engine constructs a validated, single-product handoff package with
job ID, product key, CSV and manifest payloads, an optional CAD-runtime payload,
optional table input metadata, and ordered step snapshots. The package
...
The expected-artifact inventory lists CSV, manifest, optional runtime result, and declared
...
The scheduler builds handoff packages for jobs containing `RunCADRuntime`.
Jobs without that step use direct execution. Package execution takes a defensive
snapshot of the supplied package; direct execution rejects CAD-runtime plans
because runtime dispatch requires package context.
...
Scheduling returns per-job step snapshots, errors, and CAD-runtime
...
See [adapter architecture](../adapters/README.md) for those
...
Execution errors carry product/step identity, the cause, and timeout/canceled
...
[job lifecycle](../runtime/job-lifecycle.md) for API state transitions and
