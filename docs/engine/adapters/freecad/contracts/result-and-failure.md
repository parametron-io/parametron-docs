...
A payload shape for runtime trace evidence is defined, but **no trace file is
currently written**; there is no CLI flag, path containment, or emission from
`execute`, observation, or reference traversal.
